                ...
                <div class="dropdown ms-sm-3 header-item topbar-user">
                    <button type="button" class="btn" id="page-header-user-dropdown" data-bs-toggle="dropdown" aria-haspopup="true" aria-expanded="false">
                        <span class="d-flex align-items-center">                         
                            <img class="rounded-circle header-profile-user" src="@if(Auth::check() && Auth::user()->avatar != '') {{ asset('storage/'.Auth::user()->avatar) }} @else {{ asset('assets/images/users/user-dummy-img.jpg') }} @endif" alt="Header Avatar">                          
                            <span class="text-start ms-xl-2">
                                <span class="d-none d-xl-inline-block ms-1 fw-medium user-name-text">@if (Auth::check() && Auth::user()->name != '') {{Auth::user()->name }}  @endif </span>
